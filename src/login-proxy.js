...
const COOKIE_NAME = process.env.CODEXAPP_COOKIE_NAME || "codex_app_web_session";
const SESSION_TTL_MS = Number(process.env.CODEXAPP_SESSION_TTL_MS || 12 * 60 * 60 * 1000);
...
const appEntryPath = "/";
const appName = process.env.CODEXAPP_LOGIN_APP_NAME || "Codex App";
const loginSubtitle = process.env.CODEXAPP_LOGIN_SUBTITLE || "Sign in to continue to your self-hosted Codex workspace.";
const prefillUsername = process.env.CODEXAPP_PREFILL_USERNAME === "1";
...
<head>
  ...
  <title>${escapeHtml(appName)} Login</title>
  <style>
    :root {
...
<body>
  <main>
    ...
    <p>${escapeHtml(loginSubtitle)}</p>
    ${error ? `<div class="error">${escapeHtml(error)}</div>` : ""}
    <form method="post" action="/login">
      <input type="hidden" name="next" value="${escapeHtml(next)}" />
      <label for="username">Username</label>
      <input id="username" name="username" autocomplete="username" value="${prefillUsername ? escapeHtml(username) : ""}" required />
